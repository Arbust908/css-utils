...
const originalColorCount = computed(() => [...new Set(extractedColorsList.value.flat())].length)
  ...
  colorsToTW.value
    ...
    .sort((a, b) => {
      if ((a.count > 0 && b.count > 0) || (a.count === 0 && b.count === 0)) {
        const complexA = {
          colorName: a.name?.split('-')[0].toLowerCase() || '',
          colorNum: Number(a.name?.split('-')[1]) || 0,
        }
        const complexB = {
          colorName: b.name?.split('-')[0].toLowerCase() || '',
          colorNum: Number(b.name?.split('-')[1]) || 0,
        }
        if (complexA.colorName === complexB.colorName)
          return complexA.colorNum - complexB.colorNum
  ...
  <div class="general-layout">
    ...
    <UCard class="col-span-1">
      <section class="grid grid-cols-2 gap-2">
        <div class="gpa-x-4 col-span-full flex">
          <p>Original color count: {{ originalColorCount }}</p>
          <p>Unique color count: {{ uniqueTailwindColors.length }}</p>
        </div>
        <div
